...
export function Avatar({ src, alt, size = "md", storyRing = false, className = "" }: AvatarProps) {
  const { px, textSize } = sizeMap[size];
  const ringPad = storyRing ? "p-0.5" : "";
  ...
  const inner = src ? (
    ...
    <div
      className={`flex items-center justify-center rounded-full bg-doggram-warm-white border border-doggram-border w-full h-full ${textSize}`}
    >
      🐕
    </div>
  );
  ...
  if (storyRing) {
    return (
      <div
        className={`rounded-full bg-gradient-to-tr from-doggram-coral via-doggram-orange to-doggram-amber ${ringPad} ${className}`}
